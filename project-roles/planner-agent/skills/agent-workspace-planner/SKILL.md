...
4. Create one bounded item per lane, phase, or small batch. Avoid a single oversized "do everything" item.
   ...
   For opportunity-discovery goals involving free rewards, bounties, sweepstakes, rebates, low-value manual web tasks, or "tasks for agents to find/do", create a source-first plan. The first phase must produce an actionable opportunity matrix, not a software MVP. Each research slice must name exact sources to inspect and must output opportunity cards with source URL, concrete user action, eligibility, required accounts/resources, reward range, time/cost estimate, automation fit, legal/ToS risk, CAPTCHA/KYC/payment/manual-review flags, first safe no-side-effect test, evidence freshness, and recommended next work item. Create at least one dependency-free first-phase research slice as `READY` when it has enough context to execute; leave only dependent, aggregation, owner-gated, or explicitly blocked follow-up items as `DRAFT` with `dependsOn` or `inputPacket.draftReason`. Only create execution/tool-building items after the matrix is accepted and points to concrete validated opportunities.
